...
export async function loadOtherUserProfileFromServer(
    cli: PulsarClient,
    username: string
): Promise<UserProfile> {
    const profile: UserProfile = {};
    ...
    try {
        const rsp = await cli.requestRaw(`!profile get ${username}`);
        ...
        console.error('Failed to load other user profile from server:', err);
    }
